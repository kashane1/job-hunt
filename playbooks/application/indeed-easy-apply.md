...
- Screenshot the form area ONLY (exclude browser chrome, tabs, extension popups). Write the **raw** capture to a temp path — never directly into `checkpoints/`.
...
- Screenshot the post-submit page to a temp path, then `sanitize-screenshot` it into `checkpoints/post_submit.png` (same region-derivation + sanitize as Step 5; never write the raw PNG into `checkpoints/`).
